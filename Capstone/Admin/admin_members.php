...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'upload_doc' && isset($_FILES['document'])) {
        ...
        if (in_array($ext, $allowed) && $file['size'] < 5*1024*1024) {
            ...
            $stmt->execute();
            header('Location: admin_members.php?msg=Document+uploaded.'); exit;
        }
    }
}
...
<div class="modal-overlay" id="modal-upload">
  <div class="modal">
    ...
    <form method="POST" enctype="multipart/form-data">
      ...
      <input type="hidden" name="member_id" id="uploadMemberId">
      <div class="form-group">
        <label class="form-label">Member</label>
        <input type="text" id="uploadMemberName" class="form-control" readonly>
      </div>
      <div class="form-group">
...
function openUploadModal(id, name) {
  document.getElementById('uploadMemberId').value = id;
  document.getElementById('uploadMemberName').value = name;
