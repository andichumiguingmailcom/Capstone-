...
<aside class="sidebar">
  ...
  <nav class="sidebar-nav">
    ...
    <a href="<?= appendContextToUrl('admin_pre_applications.php') ?>" class="nav-item <?= ($activePage??'') === 'pre_apps' ? 'active' : '' ?>">
    ...
    <?php endif; ?>
    <?php if (in_array($user['role'], ['general_manager','book_keeper','collector','loan_officer'], true)): ?>
    <a href="<?= appendContextToUrl('admin_loans.php') ?>" class="nav-item <?= ($activePage??'') === 'loans' ? 'active' : '' ?>">
    ...
    <a href="<?= appendContextToUrl('admin_sales.php') ?>" class="nav-item <?= ($activePage??'') === 'sales' ? 'active' : '' ?>">
      <span class="nav-icon">🛒</span> Sales
    </a>
    <?php endif; ?>
    <div class="nav-section-label">Reports & Admin</div>
    <?php if (in_array($user['role'], ['general_manager','book_keeper'], true)): ?>
    <a href="<?= appendContextToUrl('admin_reports.php') ?>" class="nav-item <?= ($activePage??'') === 'reports' ? 'active' : '' ?>">
      <span class="nav-icon">📈</span> Reports
    </a>
    <?php endif; ?>
    <?php if ($user['role'] === 'general_manager'): ?>
      <a href="<?= appendContextToUrl('admin_users.php') ?>" class="nav-item <?= ($activePage??'') === 'users' ? 'active' : '' ?>">
        <span class="nav-icon">🔧</span> User Management
      </a>
      <a href="<?= appendContextToUrl('admin_audit.php') ?>" class="nav-item <?= ($activePage??'') === 'audit' ? 'active' : '' ?>">
        <span class="nav-icon">🔍</span> Audit Logs
      </a>
    <?php endif; ?>
    <a href="<?= appendContextToUrl('../logout.php?context=admin') ?>" class="nav-item" style="color:rgba(255,100,100,0.8);">
